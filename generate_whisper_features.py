# adapted from https://github.com/HuthLab/encoding-model-scaling-laws/blob/main/extract_speech_features.py
...
import torch
import torchaudio
from transformers import PreTrainedModel, AutoFeatureExtractor, WhisperForConditionalGeneration, AutoModelForSpeechSeq2Seq, AutoProcessor, WhisperProcessor
...
def extract_speech_features(model: PreTrainedModel, processor: WhisperProcessor, model_config: dict, wav: torch.Tensor,
    ...
    is_whisper_model = isinstance(model, WhisperForConditionalGeneration)
    ...
    snippet_texts = [] # text of each snippet
    ...
    for batch_idx, (snippet_starts, snippet_ends) in snippet_iter:
        ...
        if feature_extractor is not None:
            ...
            preprocessed_snippets = feature_extractor(list(batched_wav_in.cpu().numpy()),
                                                      ...
                                                      **feature_extractor_kwargs)
            for key in preprocessed_snippets:
                preprocessed_snippets[key] = preprocessed_snippets[key].half()

            if is_whisper_model:
                chunk_features = model.model.encoder(preprocessed_snippets[features_key].to(model.device))
                ...
                contributing_outs = torch.nn.functional.conv1d(contributing_outs,
                                                               torch.ones((1,1)+model.model.encoder.conv1.kernel_size).to(contributing_outs),
                                                               stride=model.model.encoder.conv1.stride,
                                                               padding=model.model.encoder.conv1.padding,
                                                               dilation=model.model.encoder.conv1.dilation,
                                                               groups=model.model.encoder.conv1.groups)
                # shape: (batchsz, 1500)
                contributing_outs = torch.nn.functional.conv1d(contributing_outs,
                                                               torch.ones((1,1)+model.model.encoder.conv2.kernel_size).to(contributing_outs),
                                                               stride=model.model.encoder.conv2.stride,
                                                               padding=model.model.encoder.conv2.padding,
                                                               dilation=model.model.encoder.conv2.dilation,
                                                               groups=model.model.encoder.conv1.groups)

                final_output = contributing_outs[0].nonzero().squeeze(-1).max()
                generated_ids = model.generate(preprocessed_snippets[features_key].to(model.device), language='en')
            else:
                ...
                chunk_features = model.model.encoder(preprocessed_snippets[features_key].to(model.device))
                generated_ids = model.generate(preprocessed_snippets[features_key].to(model.device))
        else:
            chunk_features = model.model.encoder(batched_wav_in)
            generated_ids = model.generate(batched_wav_in)
        ...
        for out_idx, output_offset in enumerate(output_inds):
            ...
            snippet_texts.extend(transcriptions)
    ...
    return {'final_outputs': out_features, 'times': times, 'snippet_texts': snippet_texts,
...
if __name__ == "__main__":
    ...
    print('Loading model', model_hf_path, 'from the Hugging Face Hub...')
    model = AutoModelForSpeechSeq2Seq.from_pretrained(model_hf_path, output_hidden_states=True).cuda().half()
    processor = AutoProcessor.from_pretrained(model_hf_path, language='en')
    ...
    for stimulus_name, stimulus_local_path in tqdm(stimulus_paths.items(), desc='Processing stories'):
        ...
        snippet_texts_save_path = f"{features_save_path}_snippet_texts"
        ...
        extract_features_kwargs = {
            'model': model, 'processor': processor, 'model_config': model_config,
        ...
        out_features, times, snippet_texts, module_features = [extracted_features[k] for k in \
                                                ['final_outputs', 'times', 'snippet_texts', 'module_features']]
        ...
        np.savez_compressed(snippet_texts_save_path + '.npz', snippet_texts=np.array(snippet_texts))
